...
pub(super) fn generate(
    ...
    let mut qr = image::DynamicImage::ImageLuma8(
        qrcode_generator::to_image_buffer(
            input,
            QrCodeEcc::Quartile,
            image_size,
        )?,
...
fn add_logo(qr: &mut ImageBuffer<Rgba<u8>, Vec<u8>>, logo: &[u8]) {
    ...
    let mut logo = image::io::Reader::new(
        Cursor::new(logo)
...
fn add_gradient(qr: &mut ImageBuffer<Rgba<u8>, Vec<u8>>) {
    let image_size = qr.width().min(qr.height()) as usize;
    let gradient = colorgrad::CustomGradient::new().colors(&[
        colorgrad::Color::from_rgba8(100, 0, 100, 255),
        colorgrad::Color::from_rgba8(30, 5, 60, 225),
    ]).build().unwrap();
...
fn get_qr_element_size(qr: &mut ImageBuffer<Rgba<u8>, Vec<u8>>) -> usize {
    ...
    let image_size = qr.width().min(qr.height());

    let mut start = image_size;
